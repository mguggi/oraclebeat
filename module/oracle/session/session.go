...
const selector = "oracle/session"

// init registers the MetricSet with the central registry.
// The New method will be called after the setup of the module and before starting to fetch data
func init() {
	if err := mb.Registry.AddMetricSet("oracle", "session", New, oracle.ParseDSN); err != nil {
		panic(err)
	}
}
...
func New(base mb.BaseMetricSet) (mb.MetricSet, error) {
	...
	logp.Info("EXPERIMENTAL: The oracle session metricset is experimental")
	...
	stmt, err := db.Prepare(
		"select /* metricset: session */ * from gv$session")
